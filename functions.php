<?php
/**
 * Theme: Visioni Periferiche
 *
 * Theme Functions, includes, etc.
 *
 * @package visioni-periferiche
 */
...
/*  Theme setup
/* ------------------------------------ */
if ( ! function_exists( 'visioni_periferiche_setup' ) ) {

	function visioni_periferiche_setup() {

		$visioni_periferiche_bg_color_defaults = array(
			'default-color'          => '000000',
		);
		add_theme_support( 'custom-background', $visioni_periferiche_bg_color_defaults );
		...
		// Thumbnail sizes
		add_image_size( 'visioni_single', 800, 493, true ); //(cropped)
		add_image_size( 'visioni_big', 1400, 928, true ); 	//(cropped)
		add_image_size( 'artist_thumb', 300, 300, true );
		add_image_size( 'event_card', 600, 400, true );
		add_image_size( 'sponsor_logo', 200, 100, false );
		...
		load_theme_textdomain( 'visioni-periferiche', get_template_directory().'/languages' );

		// Adds support for editor color palette.
		add_theme_support( 'editor-color-palette', array(
			array(
				'name'  => __( 'Black', 'visioni-periferiche' ),
				'slug'  => 'black',
				'color'	=> '#000000',
			),
			array(
				'name'  => __( 'White', 'visioni-periferiche' ),
				'slug'  => 'white',
				'color' => '#ffffff',
			),
			array(
				'name'  => __( 'Gold', 'visioni-periferiche' ),
				'slug'  => 'gold',
				'color' => '#ffd700',
			),
			array(
				'name'  => __( 'Dark Gray', 'visioni-periferiche' ),
				'slug'  => 'dark-gray',
				'color' => '#1a1a1a',
			),
			array(
				'name'  => __( 'Light Gray', 'visioni-periferiche' ),
				'slug'  => 'light-gray',
				'color' => '#f5f5f5',
			),
		));
		...
		add_action( 'init', 'visioni_periferiche_register_blocks' );

	}

}
add_action( 'after_setup_theme', 'visioni_periferiche_setup' );


/*  Enqueue javascript front
/* ------------------------------------ */
if ( ! function_exists( 'visioni_periferiche_scripts' ) ) {

	function visioni_periferiche_scripts() {

		// Swiper JS
		wp_enqueue_script( 'swiper', 'https://unpkg.com/swiper/swiper-bundle.min.js', array(), '8.4.7', true );
		
		// all script
		wp_enqueue_script( 'visioni-script', get_template_directory_uri() . '/assets/script.js', array('swiper'),'', true );

		if ( is_singular() && get_option( 'thread_comments' ) )	{ wp_enqueue_script( 'comment-reply' ); }
	}

}
add_action( 'wp_enqueue_scripts', 'visioni_periferiche_scripts' );


/*  Enqueue css front
/* ------------------------------------ */
if ( ! function_exists( 'visioni_periferiche_styles' ) ) {

	function visioni_periferiche_styles() {

		// Swiper CSS
		wp_enqueue_style( 'swiper', 'https://unpkg.com/swiper/swiper-bundle.min.css', array(), '8.4.7' );
		
		// Main styles
		wp_enqueue_style( 'visioni-main', get_template_directory_uri().'/assets/style.min.css');
		wp_enqueue_style( 'visioni-theme', get_template_directory_uri().'/style.css');

	}

}
add_action( 'wp_enqueue_scripts', 'visioni_periferiche_styles' );
 ...
 */
function visioni_periferiche_register_blocks() {
 ...
 */
function visioni_block_categories( $categories ) {
	return array_merge(
		$categories,
		array(
			array(
				'slug'  => 'visioni-blocks',
				'title' => __( 'Visioni Periferiche Blocks', 'visioni-periferiche' ),
				'icon'  => 'screenoptions',
			),
		)
	);
}
add_filter( 'block_categories_all', 'visioni_block_categories', 10, 2 );
